...
import { ArticleCategory, getArticleCategories, addArticleCategory, updateArticleCategory, deleteArticleCategory } from '@/data/articles';
...
const IconSelectionModal = ({ 
  ...
  const availableIcons = [
    'fa-folder', 'fa-file-text', 'fa-newspaper', 'fa-book', 'fa-star', 
    'fa-tags', 'fa-tag', 'fa-bookmark', 'fa-image', 'fa-video', 
    'fa-music', 'fa-pencil-alt', 'fa-pen', 'fa-edit', 'fa-file-alt',
    'fa-file-pdf', 'fa-file-word', 'fa-file-excel', 'fa-file-powerpoint', 'fa-file-image'
  ];
...
const CategoryEditModal = ({ 
  ...
  isOpen: boolean; 
  category: Partial<ArticleCategory> | null; 
  onClose: () => void; 
  onSave: (categoryData: Omit<ArticleCategory, 'id'>) => void; 
}) => {
...
export default function ArticleCategoryManagement() {
  ...
  // 处理添加/编辑分类
  const handleSaveCategory = (categoryData: Omit<ArticleCategory, 'id'>) => {
    try {
      if (selectedCategory) {
        // 更新现有分类
        const updatedCategory = updateArticleCategory(selectedCategory.id, categoryData);
        if (updatedCategory) {
  ...
  const confirmDelete = () => {
    ...
    try {
      const deleted = deleteArticleCategory(categoryToDelete.id);
      if (deleted) {
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-5">
                ...
                <ul className="text-sm text-blue-600 dark:text-blue-400 space-y-2">
                  <li className="flex items-start">
                    ...
                    <span>添加分类：创建新的文章分类，设置名称、描述和图标</span>
                  </li>
                  <li className="flex items-start">
                    <i className="fa-solid fa-check-circle mt-1 mr-2"></i>
                  ...
                  <li className="flex items-start">
                    <i className="fa-solid fa-check-circle mt-1 mr-2"></i>
                    <span>删除分类：删除不需要的分类，该分类下的文章将自动移至默认分类</span>
                  </li>
                </ul>
              </div>
            </div>
